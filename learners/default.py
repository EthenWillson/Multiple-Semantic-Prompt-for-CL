...
class NormalNN(nn.Module):
    ...
    def learn_batch(self, train_loader, train_dataset, model_save_dir, val_loader=None, text_encoder=None, trans_dict=None):
        ...
        if need_train:
            ...
            for epoch in range(self.config['schedule'][-1]):
                ...
                for i, (x, y, task)  in enumerate(train_loader):
                    ...
                    # model update
                    loss, output= self.update_model(x, y, text_encoder=text_encoder, trans_dict=trans_dict) 
        ...
        try:
            print("Training Total time:",batch_time.sum)
    ...
    def validation(self, dataloader, model=None, task_in = None, task_metric='acc',  verbal = True, task_global=False, text_encoder=None, trans_dict=None, semantic_train=None): # 推理测试，上一级在trainer.py的task_eval()
        ...
        with torch.no_grad():
            for i, (input, target, task) in enumerate(dataloader):
                ...
                if task_in is None:
                    ...
                    #     output = model.forward(input, text_encoder=text_encoder) # vitZoo
                    #     output = self.get_logits(output=output, text_encoder=text_encoder, trans_dict=trans_dict)[:, :self.valid_out_dim]
                    # else:
                    if semantic_train == 'test':
                        output = model.forward(input, text_encoder=text_encoder, semantic_test=True)
                    else:
                        output = model.forward(input, text_encoder=text_encoder)[:, :self.valid_out_dim]
                    ...
                    if len(target) > 1:
                        if task_global:
                            # if text_encoder is not None:
                            #     output = model.forward(input, text_encoder=text_encoder)
                            #     output = self.get_logits(output=output, text_encoder=text_encoder, trans_dict=trans_dict)[:, :self.valid_out_dim]
                            # else:
                            if semantic_train == 'test':
                                output = model.forward(input, text_encoder=text_encoder, semantic_test=True)
                            ...
                            # if text_encoder is not None:
                            #     output = model.forward(input, text_encoder=text_encoder)
                            #     output = self.get_logits(output=output, text_encoder=text_encoder, trans_dict=trans_dict)[:, task_in]
                            # else:
                            if semantic_train == 'test':
                                output = model.forward(input, text_encoder=text_encoder, semantic_test=True)
                            else:
                                output = model.forward(input, text_encoder=text_encoder)[:, task_in]
    ...
    def get_logits(self, output, text_encoder, trans_dict):
        ...
        with torch.no_grad():
            ...
            image_features  = output / output.norm(dim=-1, keepdim=True)
