...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None, layers_index=None): 
        # 使用哪些层的特征
        if layers_index is None:
            layers_index = [i for i in range(self.layer_depth)]
            ...
            if prompt is not None:
                if i in layers_index:
                    ...
                    if p_list is not None:
                        try:
                            mask_feature = mask_feature + p_list[1]
                        except:
                            mask_feature = p_list[1]
                p_list = None
