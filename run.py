'''
python -u run.py --config configs/imnet-r_prompt.yaml --gpuid 0 1 2 3 --repeat 1 --overwrite 0 \
    --learner_type prompt --learner_name CODAPrompt \
    --prompt_param 100 8 0.0 \
    --log_dir outputs/ImageNet_R/10-task/coda-p

nohup python run.py --gpuid 0 1 2 3 --repeat 1 --overwrite 1 --learner_type prompt --learner_name CODAPrompt --use_clip_encoder --use_label_encoder --log_dir outputs/ImageNet_R/10-task/CLIP_CIL_text > ./log/clip_myprompt_log.log 2>&1 &


nohup python run.py --gpuid 0 1 2 3 --repeat 1 --overwrite 1 --learner_type prompt --learner_name CODAPrompt --use_clip_encoder --log_dir outputs/ImageNet_R/10-task/coda-p --config configs/imnet-r_prompt.yaml > ./log/clipL+codap+mlp_imr10.log 2>&1 &

nohup python run.py --gpuid 0 1 2 3 --repeat 1 --overwrite 1 --learner_type prompt --learner_name MSPrompt --use_clip_encoder --log_dir outputs/ImageNet_R/10-task/coda-p --config configs/imnet-r_prompt.yaml > ./log/clipL+msp+mlp_imr10.log 2>&1 &
     \
...
def create_args():
    ...
    parser.add_argument('--learner_name', type=str, default='MSPrompt', help="The class name of learner")  
    ...
    parser.add_argument('--prompt_param', nargs="+", type=float, default=[5, 8, 0, 0.2, 10, 0, 8],  # [2, 8, 0, 0.2, 20, 0, 8]
                         help="e prompt pool size, e prompt length, strenth of ortho penalty, cosine angle, select prompt, task prompt pool, task pool length")
    ...
    # Clip Encoder
    parser.add_argument('--use_clip_encoder', default=False, action='store_true', help='if use text encoder')
    parser.add_argument('--use_label_encoder', default=False, action='store_true', help='if use label encoder or mlp')
    parser.add_argument('--txt_encoder', type=str, default="CLIP", help="type of text encoder")
    parser.add_argument('--temperature', type=float, default=1.0, help="temperature")
    parser.add_argument('--clip_type', type=str, default='ViT-B/16', help="The type of clip model")# ViT-B/16、ViT-B/32、ViT-L/14

    # semantic experiment
    parser.add_argument('--semantic_exp_pure_clip', default=False, action='store_true', help='if semantic experiment') 
...
if __name__ == '__main__':
    ...
    for r in range(start_r, args.repeat):
        ...
        # train model
        if not args.semantic_exp_pure_clip:
