...
def _scaled_dot_product_attention(
    ...
    if attn_mask is not None: # not in
        attn = torch.baddbmm(attn_mask, q, k.transpose(-2, -1)) # q和k矩阵相乘和attn_mask相加
    else:
        attn = torch.bmm(q, k.transpose(-2, -1)) # 矩阵乘法 q和k都是   [tgt_len, bsz * num_heads, head_dim]   [512,257,64]  ==> attn [512,257,257] 

    attn = softmax(attn, dim=-1)
    if dropout_p > 0.0:
        attn = dropout(attn, p=dropout_p)
    # (B, Nt, Ns) x (B, Ns, E) -> (B, Nt, E)
    output = torch.bmm(attn, v) # v[512,257,64]
    return output, attn
...
def multi_head_attention_forward(
    ...
    if has_torch_function(tens_ops): # not in
        return handle_torch_function(
    ...
    #
    if not use_separate_proj_weight: # in
        assert in_proj_weight is not None, "use_separate_proj_weight is False but in_proj_weight is None"
        q, k, v = _in_projection_packed(query, key, value, in_proj_weight, in_proj_bias) # query:[257,32,1024] => q[257,32,1024]
    else:
    ...
    # prep key padding mask   not in
    if key_padding_mask is not None and key_padding_mask.dtype == torch.uint8:
        warnings.warn("Byte tensor for key_padding_mask in nn.MultiheadAttention is deprecated. Use bool tensor instead.")
        key_padding_mask = key_padding_mask.to(torch.bool)

    # add bias along batch dimension (currently second)
    if bias_k is not None and bias_v is not None: # not in
    ...
    #
    if p_list is None:
        q = q.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1) # [257,32,1024] => [512,257,64]   num_heads:16   bsz:32
        if static_k is None: # in
            k = k.contiguous().view(k.shape[0], bsz * num_heads, head_dim).transpose(0, 1)  # [257,32,1024] => [512,257,64]
        else:
            ...
            k = static_k
        if static_v is None: # in
            v = v.contiguous().view(v.shape[0], bsz * num_heads, head_dim).transpose(0, 1)   # [257,32,1024] => [512,257,64]
        else:
            # TODO finish disentangling control flow so we don't do in-projections when statics are passed
        ...
        # 在这嵌入prompt
        # B, N, C = query.shape .permute(0, 2, 1, 3)
        pk, pv = p_list
        B, p_l, p_dim = pk.shape
        pk = pk.reshape(B, p_l, num_heads, p_dim // num_heads)
        pv = pv.reshape(B, p_l, num_heads, p_dim // num_heads)
        # pk_ex = pk.unsqueeze(2).expand(B, p_l, bsz, num_heads, p_dim // num_heads)
        # pv_ex = pv.unsqueeze(2).expand(B, p_l, bsz, num_heads, p_dim // num_heads)

        # q = q.unsqueeze(0).expand(B, tgt_len, bsz, num_heads, head_dim)
        # k = k.unsqueeze(0).expand(B, k.shape[0], bsz, num_heads, head_dim)
        # v = v.unsqueeze(0).expand(B, v.shape[0], bsz, num_heads, head_dim)

        k = torch.cat((pk,k), dim=1)  # k[bs, 长度50, 头数12, 64]
        v = torch.cat((pv,v), dim=1)


        # k = torch.cat((pk_ex,k), dim=1)
        # v = torch.cat((pv_ex,v), dim=1)

        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        q = q.permute(0, 2, 1, 3)
        k = k.contiguous().view(bsz * num_heads, -1, head_dim)
        v = v.contiguous().view(bsz * num_heads, -1, head_dim)
        q = q.contiguous().view(bsz * num_heads, -1, head_dim)

    # add zero attention along batch dimension (now first)
    if add_zero_attn: # not in
    ...
    if key_padding_mask is not None: # not in
    ...
    #
    attn_output, attn_output_weights = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, p_list=p_list) # 点积注意力
    attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
    attn_output = attn_output.to(torch.float16) # [8224,1024]
